...
export function NewsCard({ article, featured = false }: { article: NewsArticle; featured?: boolean }) {
  const featuredCls = featured
    ...
    : 'flex h-full flex-col bg-white border border-line hover:-translate-y-1 hover:shadow-xl hover:shadow-ink/10'
  return (
    <article className={`transition-all duration-300 ${featuredCls}`}>
      <Link href={`/news/${article.slug}`} className={`group relative block overflow-hidden ${featured ? 'aspect-video lg:aspect-auto lg:h-full lg:min-h-[340px]' : 'aspect-video shrink-0'}`}>
      ...
      <div className={`flex flex-col ${featured ? 'justify-center p-7 lg:p-10' : 'flex-1 p-5'}`}>
        ...
        <h2 className={`${featured ? 'mt-3 text-2xl leading-snug lg:text-[38px] lg:leading-tight' : 'mt-2.5 line-clamp-2 text-[21px] leading-snug'} font-bold`}>
        ...
        <div className={`${featured ? '' : 'mt-auto'} border-t border-line pt-3 text-xs font-medium text-subtle`}>{article.author}</div>
      </div>
    </article>
  )
}
...
export function NewsGrid({ articles, cols = 3 }: { articles: NewsArticle[]; cols?: number }) {
  return (
    <div className={`grid auto-rows-fr gap-5 ${cols === 3 ? 'md:grid-cols-2 lg:grid-cols-3' : 'sm:grid-cols-2'}`}>
